...
- **CE-PE Connectivity via RIP**: RIP is used to provide routing connectivity between CE and PE devices and to enable route exchange between the customer network and the Service Provider. For internal destinations, RT24 redistributes L2 IS-IS routes into the RIP process. All RT24 internal interfaces are configured as passive-interfaces in RIP, and route-map filters are applied to ensure that only internal customer routes are advertised from RT24 toward the PEs. This design prevents RT24 from re-advertising RIP routes learned from one PE back to another, thereby avoiding the risk of the service provider core using the customer network as an unintended backup path (which could occur because RIP's Administrative Distance is typically favored over iBGP's). When PEs (CSR32 and XRV9) receive RIP routes from the CEs, they redistribute them into MP-BGP and advertise them across the MPLS core. On the remote side of the service provider network, the receiving PEs take the MP-BGP routes, redistribute them back into RIP toward the CEs, and the CEs then inject these routes into IS-IS as Level-2 routes for propagation within the internal customer network.
